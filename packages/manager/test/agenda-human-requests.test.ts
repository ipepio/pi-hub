...
describe("HumanRequestRepository — pauseRunningForHuman (P3.2, §1.4)", () => {
  ...
  it("atomicidad: si el UPDATE de la Initiative falla DENTRO de la tx, ROLLBACK revierte ambas filas", () => {
    const db = openMemoryDb();
    const repo = new HumanRequestRepository(db);
    seedRunning(db);

    // B2: el test viejo forzaba TURN_NOT_FOUND en la LECTURA del turno (paso 2), es
    // decir ANTES de cualquier UPDATE, así que su rollback era trivialmente vacío y
    // no probaba nada. Este test fuerza el fallo DENTRO del UPDATE de la Initiative
    // (paso 6), DESPUÉS de que el UPDATE del turno (paso 5) ya se ejecutó, con un
    // trigger BEFORE UPDATE que aborta. El ROLLBACK debe revertir AMBAS filas.
    db.exec(
      `CREATE TRIGGER atomicity_fail_ini
         BEFORE UPDATE ON initiatives
         BEGIN
           SELECT RAISE(ABORT, 'initiative update forced fail');
         END`,
    );
    ...
    assert.throws(
      ...
      /initiative update forced fail/,
    );

    // No debe quedar una media pausa: el turno sigue final_state IS NULL (su
    // UPDATE del paso 5 fue revertido por el ROLLBACK) y la Initiative sigue
    // running (su UPDATE abortó).
    const turn = getTurn(db, "alice", "turn-1");
    assert.equal(turn?.final_state, null, "el turno NO debe quedar paused_for_human");
    assert.equal(turn?.finished_at, null);
    const ini = getInitiative(db, "ini-1");
    assert.equal(ini.state, "running", "la Initiative sigue running");

    // Desarmar el trigger y re-verificar el happy path: con la tx única intacta,
    // la pausa funciona y deja turno paused_for_human + Initiative waiting_human.
    db.exec("DROP TRIGGER atomicity_fail_ini");
    const ok = repo.pauseRunningForHuman({
      agentName: "alice",
      initiativeId: "ini-1",
      turnId: "turn-1",
      requestId: "req-1",
    ...
    assert.equal(ok.initiativeId, "ini-1");
    assert.equal(getTurn(db, "alice", "turn-1")?.final_state, "paused_for_human");
    assert.equal(getInitiative(db, "ini-1").state, "waiting_human");
  });
